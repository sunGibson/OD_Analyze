...
object OD_trip_analyze {
  def main(args: Array[String]) {
    ...
    spark.sql("SELECT MSID FROM GSMDATA_TABLE_5 GROUP BY MSID HAVING  count(MSID) > 1")createOrReplaceTempView("GSMDATA_TABLE_6")
    //挑选出符合的数据MSID（sparksql 不支持in，exsits等操作）
    spark.sql("SELECT MSID,TIMESTAMP,X,Y,Area FROM GSMDATA_TABLE_5 A LEFT SEMI JOIN GSMDATA_TABLE_6 B ON (A.MSID=B.MSID)").createOrReplaceTempView("GSMDATA_TABLE_7")
    //挑选出GSMDATA_TABLE_5中符合条件的数据
    spark.sql("SELECT MSID,X OX,Y OY,Area OA,TIMESTAMP ENTERTIME,row_number() over(PARTITION BY MSID ORDER BY MSID,TIMESTAMP) N FROM GSMDATA_TABLE_7").createOrReplaceTempView("GSMDATA_TABLE_8")
    //对GSMDATA_TABLE_7进行分组排序并添加N(自增)
    spark.sql("SELECT ENTERTIME DEPARTTIME,OX DX,OY DY,OA DA,N,MSID ID FROM GSMDATA_TABLE_8 WHERE N > 0").createOrReplaceTempView("GSMDATA_TABLE_9")
